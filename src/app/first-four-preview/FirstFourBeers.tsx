...
export default function FirstFourBeers() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set('.fp-master-glass-viewport', { xPercent: isMobile ? 0 : -22 });
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub: 1.2,
          onUpdate: (self) => {
            const el = document.getElementById('fp-scroll-progress');
      ...
      BEERS.forEach((_, i) => {
        ...
        tl.fromTo(`.fp-bg-layer-${i}`,
          { opacity: 0, xPercent: isMobile ? 0 : (isEven ? -10 : 10) },
          { opacity: 1, xPercent: 0, duration: 0.7, ease: 'power1.inOut' },
          start + 0.3
        );

        tl.to('.fp-master-glass-viewport', {
          xPercent: isMobile ? 0 : (isEven ? -22 : 22),
          duration: 1,
          ease: 'power2.inOut',
        }, start);

        tl.to(`.fp-glass-${i - 1}`, { opacity: 0, scale: 0.8, y: -20, duration: 0.5, ease: 'power2.in' }, start);
        tl.fromTo(`.fp-glass-${i}`, { opacity: 0, scale: 1.1, y: 20 }, { opacity: 1, scale: 1, y: 0, duration: 0.5, ease: 'power2.out' }, start + 0.5);

        tl.to(`.fp-content-${i - 1}`, { opacity: 0, y: -40, duration: 0.5, ease: 'power2.in' }, start);
        tl.fromTo(`.fp-content-${i}`, { opacity: 0, y: 40 }, { opacity: 1, y: 0, duration: 0.5, ease: 'power2.out' }, start + 0.5);
      });
    }, containerRef);
  ...
  const PREMIUM_COMPOSITIONS = ['premium-lager', 'premium-whisky', 'nitro-stout', 'premium-specialty', 'premium-mead', 'premium-hefeweizen'];
  const WARM_EDGE_COMPOSITIONS = ['premium-lager', 'premium-whisky', 'premium-specialty', 'premium-mead', 'premium-hefeweizen'];

  return (
    <div className="relative bg-[#050505] text-white overflow-hidden selection:bg-[#d4af37]/30" style={{ fontFamily: "'Playfair Display', serif" }}>
      ...
      <div ref={containerRef} className="relative h-screen w-full overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          {BEERS.map((beer, i) => (
            <div
              key={`bg-${beer.id}`}
              className={`fp-bg-layer-${i} absolute inset-0`}
            ...
            >
              <Image
                ...
                style={{
                  filter: PREMIUM_COMPOSITIONS.includes(beer.composition!)
              ...
              {/* Vignette */}
              {WARM_EDGE_COMPOSITIONS.includes(beer.composition!) ? (
                <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-transparent to-black/60" />
              ) : (
                <div className="absolute inset-0 bg-gradient-to-b from-[#050505] via-transparent to-[#050505] opacity-80" />
              )}

              {/* Whisky Ale smoke effect */}
              {beer.composition === 'premium-whisky' && (
                <>
                  <div className="absolute inset-0 bg-gradient-to-t from-[#f97316]/30 via-transparent to-transparent opacity-50 mix-blend-screen animate-fp-pulse pointer-events-none" />
                  <div className="absolute inset-0 overflow-hidden pointer-events-none mix-blend-screen opacity-70">
                    <div className="absolute bottom-[10%] left-[35%] w-64 h-96 bg-[#e5e7eb]/10 blur-[60px] rounded-[100%] animate-fp-smoke-1" />
                    <div className="absolute bottom-[10%] right-[35%] w-72 h-96 bg-[#e5e7eb]/10 blur-[70px] rounded-[100%] animate-fp-smoke-2" />
                  </div>
                </>
              )}

              {/* Hefeweizen warm glow */}
              {beer.composition === 'premium-hefeweizen' && (
                <>
                  ...
                  <div className="absolute inset-0 overflow-hidden pointer-events-none mix-blend-screen opacity-50">
                    <div className="absolute top-[25%] left-[25%] w-56 h-56 bg-[#fde047]/20 blur-[70px] rounded-[100%] animate-fp-wheat-1" />
                    <div className="absolute bottom-[30%] right-[20%] w-64 h-64 bg-[#f59e0b]/15 blur-[80px] rounded-[100%] animate-fp-wheat-2" />
                  </div>
                </>
              )}

              {/* Tint for non-premium */}
              {!PREMIUM_COMPOSITIONS.includes(beer.composition!) && (
        ...
        <div className="absolute inset-0 flex items-start md:items-center justify-center z-20 pointer-events-none">
          <div className="fp-master-glass-viewport relative w-[70vw] md:w-[45vw] h-[52vh] md:h-[80vh] flex items-center justify-center will-change-transform mt-[10vh] md:mt-0">
            {BEERS.map((beer, i) => (
              <div
                key={`glass-${beer.id}`}
                className={`fp-glass-${i} absolute inset-0 flex items-center justify-center`}
                style={{ opacity: i === 0 ? 1 : 0 }}
              >
        ...
        <div className="absolute inset-0 z-30 pointer-events-none">
          {BEERS.map((beer, i) => {
            ...
            return (
              <div
                key={`content-${beer.id}`}
                className={`fp-content-${i} absolute inset-0
                ...
                <div className="w-full max-w-xl text-center md:text-left overflow-hidden">
                  ...
                  <div className="flex flex-wrap gap-2 md:gap-4 justify-center md:justify-start">
                    {beer.ingredients.map((ing, idx) => (
                      <div
                        key={idx}
                        className="px-3 md:px-8 py-1.5 md:py-2.5 rounded-full border border-[#d4af37]/20 backdrop-blur-2xl bg-[#1a0f0a]/70 shadow-[0_6px_20px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.05)] cursor-default"
                      >
                        <span className="text-[8px] md:text-xs font-bold tracking-[0.3em] md:tracking-[0.5em] uppercase text-[#d4af37]/90">
                          {ing}
                        </span>
                      </div>
                    ))}
        ...
        <div className="fixed right-6 md:right-16 top-1/2 -translate-y-1/2 h-48 md:h-64 w-px bg-white/10 z-[100]">
          <div
            id="fp-scroll-progress"
            className="w-full bg-[#d4af37] shadow-[0_0_15px_rgba(212,175,55,0.5)]"
            style={{ height: '0%' }}
          />
        </div>
      </div>
      ...
      <style jsx global>{`
        ...
        .animate-fp-smoke-1 { animation: fp-smoke-1 12s infinite ease-in-out; }
        .animate-fp-smoke-2 { animation: fp-smoke-2 15s infinite ease-in-out 2s; }

        @keyframes fp-wheat-1 {
          0%, 100% { transform: scale(1) translate(0, 0); opacity: 0.4; }
          50%       { transform: scale(1.15) translate(4vw, -3vh); opacity: 0.75; }
        }
        @keyframes fp-wheat-2 {
          0%, 100% { transform: scale(1) translate(0, 0); opacity: 0.35; }
          50%       { transform: scale(1.1) translate(-3vw, 4vh); opacity: 0.65; }
        }
        .animate-fp-wheat-1 { animation: fp-wheat-1 13s infinite ease-in-out; }
        .animate-fp-wheat-2 { animation: fp-wheat-2 17s infinite ease-in-out 3s; }
        .animate-fp-pulse   { animation: pulse 4s cubic-bezier(0.4, 0, 0.6, 1) infinite; }
      `}</style>
    </div>
  );
}
